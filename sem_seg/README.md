...
For training the models listed below, we follow the common settings used by MMSegmentation (details [here](https://github.com/open-mmlab/mmsegmentation/blob/master/docs/model_zoo.md#common-settings)), unless otherwise stated.
...
python -m bdd100k.eval.run -t sem_seg \
    -g ../data/bdd100k/labels/sem_seg/masks/${SET_NAME} \
    -r ${OUTPUT_DIR} \
...
```bash
python -m bdd100k.label.to_color -m sem_seg -i ${OUTPUT_DIR} \
